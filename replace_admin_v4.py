import xmlrpc.client
import ssl

url = "https://aulavie.xtd.es/"
db = "aulavie"
username = "admin"
password = "admin"  # The current admin password


def connect():
    ctx = ssl._create_unverified_context()
    common = xmlrpc.client.ServerProxy(f"{url}/xmlrpc/2/common", context=ctx)
    # Determine correct password
    working_password = "admin"
    uid = common.authenticate(db, "admin", working_password, {})

    if not uid:
        working_password = "xtendoo"  # Try this next
        uid = common.authenticate(db, "admin", working_password, {})
    if not uid:
        working_password = "xtendoo"
        uid = common.authenticate(db, "xtendoo", working_password, {})

    models = xmlrpc.client.ServerProxy(f"{url}/xmlrpc/2/object", context=ctx)
    return models, uid, working_password


def replace_admin():
    try:
        models, uid, password = connect()  # Unpack 3 values
        if not uid:
            print("Auth failed with both 'admin' and 'xtendoo'")
            return

        print(f"Connected with UID: {uid}, using password: {password}")

        # 1. Archive existing 'xtendoo' user (if it's not ID 2)
        xtendoo_ids = models.execute_kw(
            db, uid, password, "res.users", "search", [[("login", "=", "xtendoo")]]
        )
        print(
            f"Found existing xtendoo users: {xtendoo_ids}"
        )  # Debug which users were found
        for xid in xtendoo_ids:
            if xid != 2:
                print(f"Archiving existing 'xtendoo' user (ID {xid})...")
                models.execute_kw(
                    db,
                    uid,
                    password,
                    "res.users",
                    "write",
                    [[xid], {"login": f"xtendoo_archived_{xid}", "active": False}],
                )

        # 2. Update Admin (ID 2) to be Xtendoo
        print("Updating Admin (ID 2) to 'Xtendoo'...")
        models.execute_kw(
            db,
            uid,
            password,
            "res.users",
            "write",
            [
                [2],
                {
                    "name": "Xtendoo",
                    "login": "xtendoo",
                    "email": "manuelcalero@xtendoo.es",
                    "password": "xtendoo",
                    "lang": "es_ES",
                    # Ensure it has the correct image/preferences if needed, but this is enough strictly permissions-wise
                },
            ],
        )
        print("Admin user successfully replaced by Xtendoo.")

    except Exception as e:
        print(f"Error: {e}")


if __name__ == "__main__":
    replace_admin()
